...
;function PHPADM_ReloadZoneCode()
{
	...
	for(var i=0;i<zonesLength;i++){
		...
		addListener(zones[i], 'click', function() {
			PHPADM_XmlHttpRequest("http://219.153.58.184:81/phpad.stat/adclick.php",this.getAttribute("bannerinfo")); /// 178 ADM
	//		PHPADM_XmlHttpRequest("http://219.153.58.167:82/phpad.stat/adclick.php",this.getAttribute("bannerinfo"));///179 ADM server
		});
	}
	bannerinfoSet="["
	for (var i = 0; i < zonesLength; i++) {
	...
	bannerinfoSet+="]";
	url="http://219.153.58.184:81/phpad.stat/adlog.php"; /// 178
//	 url="http://219.153.58.167:82/phpad.stat/adlog.php";/// 179
