...
(function() {
  ...
  hm.src = "//hm.baidu.com/hm.js?8631bbdacb5b56cda3b04b81ec97983f";
...
/********åº•å±‚å›¾ç‰‡ç‚¹å‡»æ’­æ”¾*************/
H5Photo = function(obj) {
	this.__img = $(obj).find("img");
	this.__imgIndex = 0;
	this.__click = function() {
		var that = this;
		this.__img.on("click",function(){
			var mt = common.setMeta(common.getMeta("content","width=device-width, user-scalable=no"),"content","width=device-width, user-scalable=no, initial-scale=1.0");
			obj.mask();
			that.__imgIndex = that.__img.index(this);
			var imgstr = "<div class='_h5_bigImg' style='width:100%;height:100%;overflow:hidden; position:fixed; top:0; left:0; z-index:50001;'><ul style='height:100%;margin-left:"+-that.__imgIndex*$(window).width()+"px;'>";
			for (var i=0;i<that.__img.size();i++) {
				imgstr += "<li style='float:left;position:relative;width:"+$(window).width()+"px;height:100%;text-align:center;'><img src='"+that.__img.eq(i).attr("src")+"' style='width:auto !important;height:auto !important;margin:auto;diplay:block;position:absolute;left:0;top:0;right:0;bottom:0;max-width:95%;max-height:95%;'></li>";
			}
			imgstr += "</ul><label class='imginfo' style='display:block;font-size:1.5rem; position:absolute; z-index:50002; color:#fff; top:1rem; left:1rem;'></label></div>";
			$(obj).append(imgstr);
			$("._h5_bigImg label").text(that.__imgIndex+1+"\/"+that.__img.size()+"ã€€"+ (that.__img.eq(that.__imgIndex).attr("alt") || ""));
			
			$("*").css("touch-action","pan-y");
			Zepto("li img").on("touchmove",function(e) {
				e.preventDefault();
			});
			
			Zepto("._h5_bigImg ul").swipeLeft(function(e){
				that.__imgIndex = ((that.__imgIndex+1)>=that.__img.size()?that.__imgIndex:++that.__imgIndex);
				Zepto("._h5_bigImg ul").animate({marginLeft:-that.__imgIndex*$(window).width()+"px"},"fast");
				Zepto("._h5_bigImg label").text(that.__imgIndex+1+"\/"+that.__img.size()+"ã€€"+ (that.__img.eq(that.__imgIndex).attr("alt") || ""));
			}).swipeRight(function(e){
				__imgIndex = ((that.__imgIndex-1)<0?that.__imgIndex:--that.__imgIndex);
				Zepto("._h5_bigImg ul").animate({marginLeft:-that.__imgIndex*$(window).width()+"px"},"fast");
				Zepto("._h5_bigImg label").text(that.__imgIndex+1+"\/"+that.__img.size()+"ã€€"+ (that.__img.eq(that.__imgIndex).attr("alt") || ""));
			});
			
			window.addEventListener("onorientationchange" in window ? "orientationchange" : "resize", function(){
				$("._h5_bigImg ul").css("margin-left",-that.__imgIndex*$(window).width()+"px");
				$("._h5_bigImg li").css("width",$(window).width()+"px");  
			}, false);
			
			Zepto("._h5_bigImg ul").on("click",function(){
				common.setMeta(mt,"content","width=device-width, user-scalable=no");
				window.removeEventListener("onorientationchange" in window ? "orientationchange" : "resize", that.__ori, false);
				Zepto(document.body).off("touchmove");
				$(this).off("click");
				$("._h5_bigImg").remove();
				"body".unmask();
			});
	...
	String.prototype.mask = function() {
		...
		var divHtml = '<div class="_h5_divMask" style="z-index:50000;position:fixed; width:9999px; height:9999px; left:0; top:-1000px; background-color:#111111; opacity:.95; filter:alpha(opacity=95)"> </div>';
		$(that).parent().append(divHtml);
		$(that).data("mask","true");
	};
	
	String.prototype.unmask = function() {
		var that = this.toString();
		$(that).parent().find("._h5_divMask").remove();
...
H5FenYe = function(obj) {
	...
	this.LoadingIMG = "http://www.cqnews.net/images/h5/loading5.gif";
	...
	this.startLoad = function() {
		...
		if($("#_h5_content ul li").html()){
			
			
			if ($("#_h5_content ul li").last().text()=="å…¨æ–‡é˜…è¯»") {
				$("#_h5_content ul").remove();
				$("#next").remove();
				$(obj).append("<div id='next' class='more'><a href='javascript:;'><span>å±•å¼€ä¸‹ä¸€é¡µ</span></a></div>");
				var that = this;
		...
		if ($("#displaypagenum a").last().text()=="ä¸‹ä¸€é¡µ") {
	...
	this.loadNext = function(_url) {
		...
		$('#tmp').load(nexturl+" "+obj,function(responseText, textStatus,XMLHttpRequest){
			if (textStatus == "error") {
				$("#next").remove(); //éšè—åŠ è½½ä¸‹é¡µæŒ‰é’®
			}
			else {
...
(function($, undefined) {
     ...
     * @desc **el**
     * cssé€‰æ‹©å™¨, æˆ–è€…zeptoå¯¹è±¡
     * **Options**
     * - ''container'' {selector|zepto}: (å¯é€‰)æ”¾ç½®çš„çˆ¶å®¹å™¨
     * - ''content'' {Array}: (å¿…é€‰)å†…å®¹,æ ¼å¼ä¸ºï¼š\[ {href:'å›¾ç‰‡è·³è½¬URL', pic:'å›¾ç‰‡è·¯å¾„', title:'å›¾ç‰‡ä¸‹æ–¹æ–‡å­—'}, {...}\]
     * - ''viewNum'' {Number}: (å¯é€‰, é»˜è®¤:1) å¯ä»¥åŒæ—¶çœ‹åˆ°å‡ å¼ å›¾ç‰‡
     * - ''imgInit'' {Number}: (å¯é€‰, é»˜è®¤:2)åˆå§‹åŠ è½½å‡ å¼ å›¾ç‰‡
     * - ''itemRender'' {Function}: (å¯é€‰)renderæ¨¡å¼æ—¶ï¼Œä½¿ç”¨çš„è‡ªå®šä¹‰å†…å®¹æž„é€ å‡½æ•°ï¼ŒæŽ¥å—ä¸€ä¸ªä»Ž0å¼€å§‹çš„indexå‚æ•°ï¼Œè¿”å›žç©ºå€¼æ—¶æž„é€ ç»“æŸ
     * - ''imgZoom'' {Boolean}: (å¯é€‰, é»˜è®¤:false)æ˜¯å¦ç¼©æ”¾å›¾ç‰‡,è®¾ä¸ºtrueæ—¶å¯ä»¥å°†è¶…å‡ºè¾¹ç•Œçš„å›¾ç‰‡ç­‰æ¯”ç¼©æ”¾
     * - ''loop'' {Boolean}: (å¯é€‰, é»˜è®¤:false)è®¾ä¸ºtrueæ—¶,æ’­æ”¾åˆ°æœ€åŽä¸€å¼ æ—¶ç»§ç»­æ­£å‘æ’­æ”¾ç¬¬ä¸€å¼ (æ— ç¼æ»‘åŠ¨)ï¼Œè®¾ä¸ºfalseåˆ™åå‘æ’­æ”¾å€’æ•°ç¬¬2å¼ 
     * - ''stopPropagation'' {Boolean}: (å¯é€‰, é»˜è®¤:false)æ˜¯å¦åœ¨æ¨ªå‘æ»‘åŠ¨çš„æ—¶å€™é˜»æ­¢å†’æ³¡(æ…Žç”¨,ä¼šå¯¼è‡´ä¸Šå±‚å…ƒç´ æŽ¥å—ä¸åˆ°touchMoveäº‹ä»¶)
     * - ''springBackDis'' {Number}: (å¯é€‰, é»˜è®¤:15)æ»‘åŠ¨èƒ½å¤Ÿå›žå¼¹çš„æœ€å¤§è·ç¦»
     * - ''autoPlay'' {Boolean}: ((å¯é€‰, é»˜è®¤:true)æ˜¯å¦è‡ªåŠ¨æ’­æ”¾
     * - ''autoPlayTime'' {Number}: (å¯é€‰, é»˜è®¤:4000ms)è‡ªåŠ¨æ’­æ”¾çš„é—´éš”
     * - ''animationTime'' {Number}: (å¯é€‰, é»˜è®¤:400ms)æ»‘åŠ¨åŠ¨ç”»æ—¶é—´
     * - ''showArr'' {Boolean}: (å¯é€‰, é»˜è®¤:true)æ˜¯å¦å±•ç¤ºä¸Šä¸€ä¸ªä¸‹ä¸€ä¸ªç®­å¤´
     * - ''showDot'' {Boolean}: (å¯é€‰, é»˜è®¤:true)æ˜¯å¦å±•ç¤ºé¡µç 
     * - ''slide'' {Function}: (å¯é€‰)å¼€å§‹åˆ‡æ¢é¡µé¢æ—¶æ‰§è¡Œçš„å‡½æ•°,ç¬¬1ä¸ªå‚æ•°ä¸ºEventå¯¹è±¡,ç¬¬2ä¸ªå‚æ•°ä¸ºæ»‘åŠ¨åŽçš„pageé¡µç 
     * - ''slideend'' {Function}: (å¯é€‰)é¡µé¢åˆ‡æ¢å®Œæˆ(æ»‘åŠ¨å®Œæˆ)æ—¶æ‰§è¡Œçš„å‡½æ•°,ç¬¬1ä¸ªå‚æ•°ä¸ºEventå¯¹è±¡,ç¬¬2ä¸ªå‚æ•°ä¸ºæ»‘åŠ¨åŽçš„pageé¡µç 
     *
     * **Demo**
    ...
    $.ui.define('slider', {
        ...
        /**
         * è®¾ç½®è½®æ’­æ¡åŠå…ƒç´ å®½åº¦,è®¾ç½®é€‰ä¸­dot,è®¾ç½®ç´¢å¼•map,åŠ è½½å›¾ç‰‡
         */
        _setWidth:function(){
            var me = this,
                o = me._data,
        ...
        /**
         * é‡è®¾å®¹å™¨åŠå­å…ƒç´ å®½åº¦
         */
        _resize:function() {
            var me = this,
                o = me._data,
                width = o.root.offsetWidth / o.viewNum, //todo æ·»åŠ èŽ·å–éšè—å…ƒç´ å¤§å°çš„æ–¹æ³•
                length = o.length,
                items = o.items;
            if(!width) return me;
            o.width = width;
         ...
         * @name Trigger Events
         * @theme event
         * @desc ç»„ä»¶å†…éƒ¨è§¦å‘çš„äº‹ä»¶
         * ^ åç§° ^ å¤„ç†å‡½æ•°å‚æ•° ^ æè¿° ^
         * | init | event | ç»„ä»¶åˆå§‹åŒ–çš„æ—¶å€™è§¦å‘ï¼Œä¸ç®¡æ˜¯renderæ¨¡å¼è¿˜æ˜¯setupæ¨¡å¼éƒ½ä¼šè§¦å‘ |
         * | slide | event | å¼€å§‹åˆ‡æ¢é¡µé¢æ—¶æ‰§è¡Œçš„å‡½æ•°ï¼Œå‚æ•°ä¸ºæ»‘åŠ¨åŽçš„pageé¡µç  |
         * | slideend | event | é¡µé¢åˆ‡æ¢å®Œæˆ(æ»‘åŠ¨å®Œæˆ)æ—¶æ‰§è¡Œçš„å‡½æ•°ï¼Œå‚æ•°ä¸ºæ»‘åŠ¨åŽçš„pageé¡µç  |
         * | destroy | event | ç»„ä»¶åœ¨é”€æ¯çš„æ—¶å€™è§¦å‘ |
         */
    });
})(Zepto);